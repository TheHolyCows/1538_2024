...
PathPlannerTrajectory::State CowLibTrajectory::sample(
	...
	PathPlannerTrajectory::State sample = getState(low);
	PathPlannerTrajectory::State prevSample = getState(low - 1);
...
size_t CowLibTrajectory::getNextRotationTargetIdx(
...
std::vector<PathPlannerTrajectory::State> CowLibTrajectory::generateStates(
		std::shared_ptr<PathPlannerPath> path,
		const frc::ChassisSpeeds &startingSpeeds,
		const frc::Rotation2d &startingRotation) {
	std::vector < PathPlannerTrajectory::State > states;
